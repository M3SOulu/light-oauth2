...
For the version corresponding to the PROMISE25 paper "LO2: Microservice API Anomaly Dataset of Logs and Metrics",
see the [corresponding release](https://github.com/M3SOulu/light-oauth2/releases/tag/repl_v1.0.0).
The corresponding dataset is available at https://doi.org/10.5281/zenodo.14257989.
...
- [prometheus_metrics.txt](prometheus_metrics.txt) List of available and queried Prometheus metrics (from our research server)
...
To perform a single **data run**, i.e. deploy the system and execute all the locust tests, run the [data_run.sh](data_run.sh) script.

The script performs the following:
- Get the list of all tagged tasks from [locust](locust) files
  ...
  - `LO2_run_UNIX`: root folder of data, `UNIX` is the unix timestamp of the beginning of the run
    ...
    - `correct`/`ERROR`: directory for the `correct` or `correct+ERROR` test execution
      ...
      - `metrics`: folder containing all data from Prometheus and Jaeger
        - `metric_*.json`: a JSON file for each Prometheus metric from [prometheus_metrics.txt](prometheus_metrics.txt) with metric values
...
Bakhtin, A., Nyyssölä, J., Wang, Y., Ahmad, N., Ping, K., Esposito, M., Mäntylä, M., & Taibi, D. (2025).
LO2: Microservice API Anomaly Dataset of Logs and Metrics.
Proceedings of the 21st International Conference on Predictive Models and Data Analytics in Software Engineering, 1–10.
https://doi.org/10.1145/3727582.3728682
