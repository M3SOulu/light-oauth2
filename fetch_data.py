...
def save_jaeger_traces(output_directory, service_name, traces):
    ...
    with open(file_path, 'a', newline='') as file:
        ...
        for trace in traces['data']:
            for span in trace.get('spans', []):
                ...
                end_time = datetime.fromtimestamp((span.get('startTime') + duration_micros) / 1e6,
...
if __name__ == "__main__":
    ...
    for metric_name in metric_names:
        ...
        if metrics_data:
            ...
            print(f"Failed to save metrics for {metric_name}")

    # Fetch and save Jaeger traces
    for service_name in JAEGER_SERVICES:
        traces = fetch_jaeger_traces(jaeger_url, service_name, start_time, end_time)
        if traces and 'data' in traces:
            save_jaeger_traces(output_directory, service_name, traces)
        else:
